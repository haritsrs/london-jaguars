...
export function ArchivePlayerCard({ player, archive, href, variant = "grid" }: { player: ArchiveCardPlayer; archive: "legends" | "past"; href?: string; variant?: "grid" | "list" }) {
  ...
  return href ? <Link className={`archive-player-card archive-player-card--${variant}`} href={href}>{content}</Link> : <article className={`archive-player-card archive-player-card--${variant}`}>{content}</article>;
}
