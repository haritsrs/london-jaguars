...
const artDirection: Record<string, { number: string; stamp: string; label: string }> = {
  "First team": { number: "01", stamp: "FIRST / TEAM", label: "2028/29 · FIRST TEAM" },
  "Match report": { number: "5–2", stamp: "COVENTRY / AWAY", label: "PREMIER LEAGUE" },
  Features: { number: "90", stamp: "TRAINING / GROUND", label: "TRAINING GROUND" },
  "Club news": { number: "18", stamp: "CLUB / UPDATE", label: "CLUB NEWS" },
  History: { number: "27/28", stamp: "2027/28 / TREBLE", label: "CLUB ARCHIVE" },
  Kits: { number: "28/29", stamp: "NIKE / LONDON", label: "KIT COLLECTION" },
  Player: { number: "11", stamp: "PLAYER / FEATURE", label: "PLAYER FEATURE" },
  Club: { number: "LDN", stamp: "LONDON / JAGUARS", label: "LONDON JAGUARS" },
};
...
export function StoryCard({ item, featured = false, compact = false }: { item: NewsItem; featured?: boolean; compact?: boolean }) {
  ...
  const readLabel = item.category === "Match report" ? "Read the report" : "Read more";
